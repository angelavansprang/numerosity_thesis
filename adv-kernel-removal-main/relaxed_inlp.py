...
def evaluate(w, X, Y, verbose=True):
    ...
    if verbose:
        print(
            "Is Projection: {}; is symmetric: {}".format(
                np.allclose(P @ P - P, 0.0), np.allclose(P - P.T, 0.0)
            )
        )
    scores = []
    losses = []
    ...
    if verbose:
        ...
        print(
            "\t\tMean: {}; Median: {}; Min: {}; Max: {}; STD: {}".format(
                np.mean(scores),
                np.median(scores),
                np.min(scores),
                np.max(scores),
...
def solve_fantope_relaxation_fr(
    X,
    Y,
    d=1,
    init_beta=None,
    ...
    def get_loss_fn(X, Y, alpha, beta, bce_loss_fn, optimize_beta=False):
        ...
        bce = (
            bce_loss_fn(alpha(X @ (I - beta)).squeeze(), Y)
            + ALPHA * alpha.weight.data.norm() ** 2
        )
        if optimize_beta:
            bce = -bce
    ...
    optimizer_w = ExtraSGD(
        w.parameters(), lr=0.1, momentum=0.9
    )  # torch.optim.SGD(w.parameters(), lr = 1e-3, momentum = 0.6)
    optimizer_adv = ExtraSGD(
        [beta], lr=0.1, momentum=0.9
    )  # torch.optim.SGD([beta],  lr = 1e-3, momentum = 0.6)
    ...
    for i in range(out_iters):
        ...
        for j in range(in_iters_adv):
            ...
            loss_adv = get_loss_fn(
                X_batch, Y_batch, w, symmetric(beta), bce_loss_fn, optimize_beta=True
            )
            loss_adv.backward()
            optimizer_adv.step()
        ...
        for j in range(in_iters_clf):
            ...
            loss_w = get_loss_fn(
                X_batch, Y_batch, w, symmetric(beta), bce_loss_fn, optimize_beta=False
            )
            loss_w.backward()
        ...
        if i % 50 == 0:
            diffs_ws = np.mean(
                [np.linalg.norm(ws[i] - ws[i - 1]) for i in range(1, len(ws))]
            )
            diffs_advs = np.mean(
                [np.linalg.norm(advs[i] - advs[i - 1]) for i in range(1, len(advs))]
            )
            ...
            if score < best_score:
                best_adv, best_score = (
                    symmetric(beta).detach().cpu().numpy().copy(),
                    score,
                )
            grads_alpha = []
            grads_beta = []
...
def solve_fantope_relaxation(
    ...
    print(
        "Dataset size: {}; device: {}; epsilon: {}; noise-std: {}, lr: {}; average feature size: {}; dropout: {}".format(
            X.shape, device, epsilon, noise_std, lr, np.mean(np.abs(X)), input_dropout
        )
    )
    ...
    def init_classifier(w):
        ...
        for j in range(1000):
            ...
            loss_w = get_loss_fn(
                X_batch, Y_batch, w, symmetric(beta), optimize_beta=False
            )
            loss_w.backward()
            optimizer_w.step()
            if j % 100 == 0:
    ...
    def solve_linear_minimization(grad, iters=15):
        ...
        optimizer = torch.optim.Adam(
            [s]
        )  # torch.optim.SGD([s], lr = 1e-3, momentum = 0.8)
        for i in range(iters):
            optimizer.zero_grad()
            loss = (grad * s).mean()
            loss.backward()
    ...
    if num_labels == 2:
        w = torch.nn.Linear(X.shape[1], 1).to(
            device
        )  # torch.randn(X.shape[1]).float().to(device)*1e-1
    else:
    ...
    if use_adam:
        ...
        optimizer_adv = torch.optim.SGD(
            [beta], lr=lr, momentum=momentum, weight_decay=weight_decay
        )  # , momentum = 0.0)
        ...
        for i in range(out_iters):
            for j in range(in_iters_adv):
                ...
                loss_adv = get_loss_fn(
                    X_batch,
                    Y_batch,
                    w if (not replay or not prev_clfs) else random.choice(prev_clfs),
                    symmetric(beta),
                    bce_loss_fn,
                    optimize_beta=True,
                )
            ...
            if i % evalaute_every == 0:
                if verbose:
                    ...
                    diffs_advs = np.mean(
                        [
                            np.linalg.norm(advs[i] - advs[i - 1])
                ...
                print(
                    "{}/{}. Acc post-projection: {:.3f} %; best so-far: {:.3f} %; Gap: {:.3f} %; Maj: {:.3f} %".format(
                        i,
                        out_iters,
                        score * 100,
                        min(best_score, score) * 100,
                        np.abs(best_score - maj) * 100,
                        maj * 100,
                    )
                )
...
if __name__ == "__main__":
    ...
    ws, advs, best_adv = solve_fantope_relaxation(
        X, Y, d=1, init_beta=None, device="cpu", out_iters=50000, in_iters=1
    )
