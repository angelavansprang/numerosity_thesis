...
gammas, degrees, alphas, alphas_sigmoid, gammas_sigmoid, gammas_laplace, gammas_rbf = (
    run_kernels.gammas,
    run_kernels.degrees,
    run_kernels.alphas,
    run_kernels.alphas_sigmoid,
    run_kernels.gammas_sigmoid,
    run_kernels.gammas_laplace,
    run_kernels.gammas_rbf,
)

# kernels = run_kernels.kernels
kernels = {
    "poly": {"torch": poly_kernel, "np": poly_kernel},
}  # only results for the poly kernel so far
...
def learn_multiple_kernels(
    ...
    if not equal_weighting:
        print("len(KLtr): ", len(KLtr))
        # print("Kltr.shape: ", KLtr[0].shape)
        # print("len(y): ", len(y))
        NN = 50000  # added this, to fix bug
        y = y[:NN]
        clf = EasyMKL().fit(KLtr, y)
        print("Done with fitting EasyMKL")
        weights = clf.solution.weights.detach().cpu().numpy()
        print("Done with detaching weights")
    else:
...
if __name__ == "__main__":
    ...
    #     easymkl_func = AveragedKernel(
    #         multiple_weights["EasyMKL"], kernel2params, kernels
    #     )
    #     unifommk_func = AveragedKernel(
    #         multiple_weights["UniformMK"], kernel2params, kernels
    #     )
    #     print("Done!")
    ...
    for filename in os.listdir(
        ...
        params_dict = {
            kv.split("=")[0]: kv.split("=")[1] for kv in params_str.split("_")
        }

        gamma, degree, alpha = (
            float(params_dict["gamma"]) if params_dict["gamma"] != "None" else 1,
            int(params_dict["degree"]) if params_dict["degree"] != "None" else 0,
        ...
        easymkl_func_best = learn_multiple_kernels(
            kernels, kernels2params_test, Z, Y, equal_weighting=True
        )  # to fix errors
        ...
        for kernel_type in [args.kernel_type]:
            if kernel_type == "mlp":
                ...
                if kernel_type == "EasyMKL":
                    ...
                    model = SVC(
                        kernel=partial(laplace_kernel, gamma=0.3)
                    )  # 1.0/Z.shape[1]))
            ...
            params_str = params_str.replace(
                ".pickle",
                "_preimage-error={}.pickle".format(error / mean_norm_normalized),
            )
            preimage_err_str, post_proj_acc_str = (
                "{:.3f}".format(error * 100 / mean_norm_normalized),
                "{:.3f}".format(100 * post_proj_acc),
            )
            params_str = params_str.replace(
                ".pickle", "_preimage-error={}.pickle".format(preimage_err_str)
            )
            params_str = params_str.replace(
                ".pickle", "post-proj-acc={}.pickle".format(post_proj_acc_str)
            )
            params2score[
                params_str.replace("_unit-vecs=False", "")
                + "_"
                + "adv-type={}".format(kernel_type)
            ] = score
