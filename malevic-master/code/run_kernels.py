# Note that most of this code is taken from the GitHub belonging to the original paper:
# https://github.com/shauli-ravfogel/adv-kernel-removal
# Changed some things, mainly: only run 1 type of kernel
...
def load_ViTpatches(dataset, objective, layer=0, threshold_patches=30, normalize=0):
    """Input:
    objective (str): "color" or "shape"
    """
    data = []

    for split in ["train", "val", "test"]:
        print(f"Started with loading ViT patches in split: {split}")
        file_name_patches = (
        ...
        file_name_reprs = (
            f"../data/{dataset}/representations/{dataset}_{split}_visual.pickle"
        )
        with open(file_name_reprs, "rb") as f:
            repr = pickle.load(f)

        X, Y = [], []
        labels = utils.make_labels_dict(dataset, split=split)
        ...
        for img_id, repr in repr.items():
            ...
            if len(nodes) <= threshold_patches:
                input = utils.filter_repr(
                    layer,
                    nodes,
                    repr,
                    single_patch=True,
                    padding_up_to=threshold_patches,
                )
                for i, patch_id in enumerate(nodes):
                    patch = input[i]
                    boxes = patches[patch_id]
                    if len(boxes) > 1:  # only consider patches with one object
                        continue
                    box = boxes[0]
...
def project_2d(X, y, method="pca", M=750, title="", mode="glove"):
    ...
    plt.savefig(
        "plots/{}/kernels/tsne-pca/{}.method={}.png".format(mode, title, method),
        dpi=600,
    )
    plt.clf()
...
def calc_nystrom_old(X, d, kernel_func, alpha, gamma, degree, eps=1e-7):
    ...
    W = kernel_func(
        X1, X1, gamma=gamma, degree=degree, alpha=alpha
    )  # W is degree x degree
    X2X1 = kernel_func(
        X2, X1, gamma=gamma, degree=degree, alpha=alpha
    )  # N-degree x degree
...
def calc_preimage_nystrom_mse(
    ...
    phi_X_torch = (
        kernel_func(X_torch, X1_torch, gamma=gamma, degree=degree, alpha=alpha)
        @ S_torch
    )
    phi_X_dev_torch = (
        kernel_func(X_dev_torch, X1_torch, gamma=gamma, degree=degree, alpha=alpha)
        @ S_torch
    )
    ...
    for i in range(N_TOTAL):
        ...
        phi_Z = (
            kernel_func(mlp(inp), X1_torch, gamma=gamma, degree=degree, alpha=alpha)
            @ S_torch
        )
        ...
        if i % 1000 == 0:
            with torch.no_grad():
                ...
                phi_Z_dev = (
                    kernel_func(
                        mlp(inp), X1_torch, gamma=gamma, degree=degree, alpha=alpha
                    )
                    @ S_torch
                )
                ...
                print(
                    "{}/{}; dev loss: {}; train loss: {}".format(
                        i, N_TOTAL, mean_loss_dev[-1], np.mean(mean_loss)
                    )
                )
                # exit()
                mlp.train()
    ...
    with torch.no_grad():
        ...
        phi_Z_dev = (
            kernel_func(
                best_mlp(inp), X1_torch, gamma=gamma, degree=degree, alpha=alpha
            )
            @ S_torch
        )
    ...
    mean_norm_normalized = (
        (phi_X_dev_debiased_torch.norm(dim=1)).mean().detach().cpu().numpy()
    )
...
def learn_multiple_kernels(
    kernels, kernel2params, X, y, numpy=False, equal_weighting=False
):
                ...
                for k, alpha in enumerate(kernel2params[kernel_type]["alphas"]):
                    ...
                    KLtr.append(
                        kernels[kernel_type]["np"](
                            X, degree=degree, gamma=gamma, alpha=alpha
                        )
                    )
                    kernels_lst.append(
                        {
                            "func_np": kernels[kernel_type]["np"],
                            "func_torch": kernels[kernel_type]["torch"],
                            "degree": degree,
                            "gamma": gamma,
                            "alpha": alpha,
                        }
...
# gammas = [0.05, 0.1, 0.15] # original
gammas = [0.1]
# alphas = [0.8, 1, 1.2] # original
alphas = [1]
gammas_sigmoid = [0.005, 0.003]
gammas_rbf = [0.1, 0.15, 0.2]
gammas_laplace = gammas_rbf
alphas_sigmoid = [0, 0.01]
# degrees = [2, 3] # original
degrees = [2]
do_normalize = [False]

kernels = {
    "poly": {"torch": poly_kernel, "np": poly_kernel},
    "rbf": {"torch": rbf_kernel_torch, "np": rbf_kernel},
    "laplace": {"torch": laplace_kernel_torch, "np": laplace_kernel},
    "linear": {"torch": linear_kernel, "np": linear_kernel},
    "sigmoid": {"torch": sigmoid_kernel_torch, "np": sigmoid_kernel},
}
...
if __name__ == "__main__":
    ...
    np.random.seed(0)
    X, Y = X[:NN], Y[:NN]
    print("Limit to first 50,000 data points, remaining:")
    print("len(X): ", len(X))
    print("len(Y): ", len(Y))
    ...
    # if len(kernels) > 1:
    #     K_multiple_torch, K_multiple_np, _, _ = learn_multiple_kernels(
    #         kernels, kernel2params, X, Y, equal_weighting=False
    #     )
    #     kernels["EasyMKL"] = {"torch": K_multiple_torch, "np": K_multiple_np}
    #     kernel2params["EasyMKL"] = {
    #         "gammas": [None],
    #         "degrees": [None],
    #         "alphas": [None],
    #     }
    #     K_multiple_torch_eq, K_multiple_np_eq, _, _ = learn_multiple_kernels(
    #         kernels, kernel2params, X, Y, equal_weighting=True
    #     )
    #     kernels["UniformMK"] = {"torch": K_multiple_torch_eq, "np": K_multiple_np_eq}
    #     kernel2params["UniformMK"] = {
    #         "gammas": [None],
    #         "degrees": [None],
    #         "alphas": [None],
    #     }
    #     K_multiple_torch_eq_rbf, K_multiple_np_eq_rbf, _, _ = learn_multiple_kernels(
    #         {"rbf": kernels["rbf"]}, kernel2params, X, Y, equal_weighting=False
    #     )
    #     kernels["UniformMK-RBF"] = {
    #         "torch": K_multiple_torch_eq_rbf,
    #         "np": K_multiple_np_eq_rbf,
    #     }
    #     kernel2params["UniformMK-RBF"] = {
    #         "gammas": [None],
    #         "degrees": [None],
    #         "alphas": [None],
    #     }
    #     with open(
    #         "../kernel_removal/interim/{}{}/kernel/multiple/multiple-kernels.pickle".format(
    #             mode, run_id
    #         ),
    #         "wb",
    #     ) as f:
